module "resource_groups" {
  ...
  resource_group_name = join("-", ["rg","net","prod","corp",var.location_prefix,each.value.name])
...
module "vnet" {
    ...
    vnet_name                   = join("-", ["vnet","prod","corp",var.location_prefix])
...
module "peering" {
  ...
  src_vnet_name = module.vnet["corp"].vnets[0].name
  dest_vnet_name = data.terraform_remote_state.conn.outputs.vnet.name
  src_vnet_id = module.vnet["corp"].vnets[0].id
