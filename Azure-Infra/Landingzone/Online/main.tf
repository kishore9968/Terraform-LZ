module "resource_groups" {
  ...
  resource_group_name = join("-", ["rg","net","prod","online",var.location_prefix,each.value.name])
  tags                = each.value.tags
  location            = var.location
  #location_prefix     = var.location_prefix local.rg_name_prefix,vnet-prod-online-eus2-001
}
...
module "vnet" {
    ...
    vnet_name                   = join("-", ["vnet","prod","online",var.location_prefix])
...
module "peering" {
  ...
  src_vnet_name = module.vnet["online"].vnets[0].name
  dest_vnet_name = data.terraform_remote_state.conn.outputs.vnet.name
  src_vnet_id = module.vnet["online"].vnets[0].id
